...
logging.getLogger('werkzeug').addFilter(HealthCheckFilter('GET /monitoring/status HTTP'))
...
class Monitoring(http.Controller):
    ...
    def status(self):
        ...
        return werkzeug.wrappers.Response(json.dumps(self.get_status()), headers=headers)
